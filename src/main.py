...
if PROJECT_ORG and PROJECT_NUMBER and gh_enabled:
    gh_tmp = GithubHandler(github_token, GH_REPO or "")
    PROJECT_ID = gh_tmp.get_project_node_id(PROJECT_ORG, int(PROJECT_NUMBER), is_org=True)
    if not PROJECT_ID:
...
@app_commands.describe(status="Show all, open (non-finished), or closed (finished) challenges.")
@app_commands.choices(status=[
    app_commands.Choice(name="All", value="all"),
    app_commands.Choice(name="Open (non-finished)", value="open"),
    app_commands.Choice(name="Closed (finished)", value="closed")
])
async def issues(interaction: discord.Interaction, status: app_commands.Choice[str]):
    ...
    rows = [f"• [{issue.title}]({issue.html_url})" for issue in issues][:10]
    msg = f"Challenges ({status.name}):\n" + "\n".join(rows)
    if len(issues) > 10:
